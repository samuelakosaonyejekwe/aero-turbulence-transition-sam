...
para("Where Squire-Young is evaluated is a choice, and it is worth more than most of the "
 ...
 "different statement. The station cannot simply be moved to the trailing edge, where "
 "Squire-Young wants it, because this section has a FINITE-ANGLE trailing edge — 26.8° "
 "included, measured off the section — so the trailing edge is a stagnation point of the "
 "inviscid flow and U_e goes to zero there physically. The panel method's collapse is the "
 "answer, not an artefact, and (U_e/U_∞)^((H+5)/2) degenerates. The evaluation station is "
 "therefore an irreducible part of applying this formula to a wedge trailing edge rather than "
 "a tuning constant, and the table bounds what it is worth. Taking the last attached station "
 "instead is worse: the drag peaks near 0.98c and falls beyond it, so that rule lands past a "
 "maximum and makes the answer depend on where Head's clamp bites. What would resolve it is a "
 "wake march — carrying the momentum integral past the trailing edge with no wall until the "
 "pressure has recovered, as a coupled panel code does — and that is a formulation extension, "
 "not a correction."
...
para("And whether the shape factor at that station is solved. Head's entrainment method has no "
 ...
 "from a bound rather than from a solved shape factor. That is the same wedge trailing edge: "
 "a layer decelerating into a stagnation point genuinely approaches separation, so the clamp "
 "is reached for a physical reason and it is Squire-Young's thin-attached-layer premise that "
 "fails, not the march. This report already flagged exactly that hazard for the trailing-edge "
 "separation margin and had not for the drag, which is the headline number; H_sy_at_clip now "
 "says which it is, per surface and per point."
