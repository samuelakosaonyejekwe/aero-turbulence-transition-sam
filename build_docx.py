...
para("There is an explanation the two experiments themselves support, and it is not the one this "
 "report reached by elimination. TN D-338 reports its OWN crossflow Reynolds numbers: "
 "\"The critical values of crossflow Reynolds number for vortex formation found in this "
 "investigation range from about 135 to 190\", while \"the values of crossflow Reynolds "
 "number for beginning transition were found to be between 190 and 260.\" The values this "
 "work's surrogate requires — %.0f on Dagenhart & Saric and %.0f on Boltz et al. — fall in "
 "those two ranges respectively. That is consistent with the two datasets marking two "
 "DIFFERENT events, vortex formation against the beginning of transition, rather than with "
 "one criterion needing two constants. It is corroboration and not a calibration: the "
 "surrogate is Re_theta2 = 0.47 Re_theta sin(L), and whether that is on the same scale as "
 "the crossflow Reynolds number TN D-338 forms is not established here. But it is external "
 "evidence, it points at a mechanism, and the receptivity attribution below was reached "
 "without it."
 % (_dag.mean_critical_value, _bol.mean_critical_value), italic=True, size=10)
