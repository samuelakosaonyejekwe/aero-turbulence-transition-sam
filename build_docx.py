...
bullet("Natural / Tollmien–Schlichting (TS): amplification of viscous instability waves in "
       "low-disturbance free flight, governed by the growth of the amplification "
       "envelope and predicted here by an e^N integral in Re_theta.")
...
bullet("Inviscid edge flow: a constant-strength vortex-panel method (Kuethe–Chow) returns the "
       "surface pressure C_p and edge velocity U_e, with a Prandtl-Glauert correction "
       "applied to C_p and the integrated loads at the cruise Mach number. The "
       "boundary-layer closures downstream are incompressible formulations and the "
       "edge velocity passed to them is left uncorrected.")
bullet("Laminar boundary layer: Thwaites' integral method advances θ, H and Re_θ from the "
       "stagnation point along each surface.")
...
para("Bypass onset uses the Abu-Ghannam & Shaw correlation evaluated at the local Tu; natural/TS onset integrates the envelope amplification rate of Drela & Giles in Re_theta and triggers at N_crit; "
 "envelope; separation-induced and cross-flow onsets use bubble and C1 criteria. The kernel "
 "takes the minimum effective onset across all four:")
for k in ["E08","E09","E10","E11","E12","E13","E14"]: equation(k)
...
for k in ["E20","E21","E22","E23","E24"]: equation(k)
...
para("Calibration. The solver is calibrated to the present case study through the single "
 "constant set of Table 7. The critical amplification factor is not among the constants: it "
 "follows from the free-stream turbulence intensity by Mack's correlation, returning 9.00 at "
 "the cruise level of 0.07 %. (AGS "
 "weight a_BP = 1, cross-flow constant C1 = 150). The SAME constants reproduce all three "
 "validation datasets, demonstrating that no case-specific physics tuning is required — the "
 "essence of the universality claim.")
