...
para("The T3C4 residual is in the bubble, and @@TAB:bubble@@ localises it. Across the dead-air "
 ...
 "0.011 m/s, which is the precision they are quoted to, and their gradient across the bubble "
 "to 0.5 %. The shape factor is not: the march reaches 3.44 against a measured 5.17, because "
 "it is bounded by the attached Falkner-Skan branch, and that cap alone accounts for a "
 "factor 1.32 of the 2.3-fold shortfall in dθ/dx. The rest is length — the modelled bubble "
 "is 0.082 m against a measured lower bound of 0.10 m — and the length is N_crit·θ_s/σ with "
 "all three computed rather than fitted, so there is no constant to adjust. Relaxing the "
 "shape-factor cap onto the reverse-flow profile the amplification rate is already read "
 "from, which removes an inconsistency in how the model describes the same region, moves "
 "T3C4 by 0.8 percentage points and the 86 aerofoil conditions by 0.0004c; it is exposed as "
 "cal[\"bub_rev_H\"] and left off, because the effect is inside the discretisation scatter "
 "and the shipped setting is the better-validated one.")
