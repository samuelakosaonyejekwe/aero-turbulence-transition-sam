...
def setup_tables():
    ...
    calrows=[
        ...
        ("len_re_x",CAL["len_re_x"],
         "transition-length correlation stated in Re_x rather than in Re_theta (ablation path only)",
         "False is the model; the two are the same law - Dhawan & Narasimha's Re_lambda = 9 Re_x^0.75 IS constant-spot-rate Re_lambda = 16.63 Re_theta^1.5 on a flat plate - and Re_theta is the form that stays exact in a pressure gradient"),
        ("bub_sigma_local",CAL["bub_sigma_local"],
         "dead-air amplification rate read at the marched H instead of at the developed reverse-flow profile (ablation path only)",
         "False is the model; True lengthens the T3C4 bubble towards the measurement but collapses the 86 NLF(1)-0416 conditions from 50 to 21 inside the bracket"),
